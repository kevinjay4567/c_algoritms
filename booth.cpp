...
template <size_t N> long long bitsetToDecimal(const std::bitset<N> &bits) {
...
int16_t toSigned(const std::bitset<BITS> &b) {
  return static_cast<int16_t>(b.to_ulong());
}

std::bitset<BITS> toBitset(int16_t x) {
  return std::bitset<16>(static_cast<uint16_t>(x));
}

std::bitset<BITS> subtract(const std::bitset<BITS> &a,
                           const std::bitset<BITS> &b) {
...
std::bitset<BITS + 1> bitsetAdd(std::bitset<16> x, std::bitset<16> y) {
  bool carry{};
  std::bitset<BITS + 1> ans;
...
// Convierte entero → binario de 16 bits (2's complement)
...
/**
 * Multiplicacion de binarios a 16 bits
 * usando el algoritmo de Booth para numeros sin signo
 */
std::bitset<32> unsigned_booth_multiply(std::bitset<16> m, std::bitset<16> q) {
  ...
  while (count > 0) {
    ...
    std::cout << std::left << "| " << std::setw(6) << count << " | "
              << std::setw(3) << q_0 << " | " << std::setw(3) << cf << " | "
              << std::setw(16) << al.to_string() << " | " << std::setw(16)
              << dl.to_string() << " |\n";
  ...
  uint32_t result = (static_cast<uint32_t>(al.to_ulong()) << 16) |
...
/**
 * Multiplicacion de binarios a 16 bits
 * usando el algoritmo de Booth para numeros con signo
 */
...
int main() {
  ...
  cout << "Ingrese el primer número (decimal o binario prefijado con b: ej: "
          "b1011): ";
  cin >> inputA;

  cout << "Ingrese el segundo número (decimal o binario prefijado con b: ej: "
          "b1100): ";
  cin >> inputB;
